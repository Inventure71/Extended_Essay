    ...
    while True:
        for player in [player1, player2]:
            ...
            if check_for_tris(board, 1 if player == player1 else -1):
                return player.name, move_history
                    ...
                    for opp_layer_size in layers_sizes:
                        for opp_player_num in ['Player1', 'Player2']:
                            ...
                            for _ in range(num_games):
                                winner, moves = play_game(current_player, opponent)  # Capture the moves as well
                            ...
                            results.append({
                                ...
                                'Losses': losses,
                                'Draws': draws,
                                'Moves': moves  # Added to record the moves for the match
                            })
    df = pd.DataFrame(results)
    df.to_csv('special_benchmark_results.csv', index=False)


def create_ranking_and_chart(df):
    # For "Model" as the primary
    model_df = df.groupby('Model').sum()[['Wins', 'Draws', 'Losses']].reset_index()

    # For "Opponent" as the primary
    opponent_df = df.groupby('Opponent').sum()[['Losses', 'Draws', 'Wins']].reset_index()
    opponent_df.columns = ['Model', 'Wins', 'Draws', 'Losses']

    # Combine both
    combined_df = pd.concat([model_df, opponent_df], axis=0)
    ranking = combined_df.groupby('Model').sum().reset_index()
    ...
    # Using bottom parameter for stacking the bars
    plt.bar(ranking['Model'], ranking['Wins'], label='Wins', color='green')
    plt.bar(ranking['Model'], ranking['Draws'], bottom=ranking['Wins'], label='Draws', color='gray')
    plt.bar(ranking['Model'], ranking['Losses'], bottom=ranking['Wins'] + ranking['Draws'], label='Losses', color='red')

    plt.xlabel('Model')
    plt.ylabel('Number of Matches')
    plt.title('Model Ranking by Wins, Draws, and Losses')
    plt.legend(loc='upper right')
    plt.xticks(rotation=90)
    plt.tight_layout()

    # Save the plot
    plt.savefig('model_ranking_chart.png')
    plt.close()


# Read the benchmark_results.csv to a DataFrame
df = pd.read_csv('models_useful_data/Tris/ELABORATING/special_benchmark_results.csv')

# Create ranking and chart
create_ranking_and_chart(df)
